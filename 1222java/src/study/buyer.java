...
public class buyer {
	...
	String total_price() {
		// 장바구니에 담겨있는 물건의 총 금액 구하고 천단위 콤마로 변환하여 return하세요
		...
		DecimalFormat basket_df = new DecimalFormat("###,###");
		return basket_df.format(total) + "원";
	 ...
	 void basket_add(mart item) {
		if (cnt >= 1) {
			...
			basket = temp; // 새로운 배열의 주소를 받아 기본 배열에 버리기
		}
		basket[cnt] = item;
		cnt++;
	}
