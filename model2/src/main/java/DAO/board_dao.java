...
public class board_dao extends parent_dao {
	...
	// 답글 저장
	public void insertreply(reply b) {
		String sql="insert into reply(board_id, comment, writer, member_id) values(?,?,?,?)";
		try {
			pt=conn.prepareStatement(sql);
			pt.setInt(1, b.getBoard_id());
			pt.setString(2, b.getComment());
			pt.setString(3, b.getWriter());
			pt.setInt(4, b.getMember_id());
			pt.executeUpdate();
			
		}catch(SQLException e){
			System.out.println("답글 데이터저장 실패");
			e.printStackTrace();
		}
	}
	...
	public board selectDetail(int id) {
		...
		try {
			...
			rs = pt.executeQuery();
			if(rs.next()) {
				return new board(rs.getInt(1),rs.getInt(7),rs.getInt(9), rs.getString(2),
						rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(8),rs.getTimestamp(6));
			}
		}catch(SQLException e) {
	...
	public List<board> Allselect(int row){
		...
		try {
			...
			rs = pt.executeQuery();
			while(rs.next()) {
				board data = new board(rs.getInt(1),rs.getInt(7),rs.getInt(9), rs.getString(2),
						rs.getString(3),rs.getString(4),rs.getString(5),
						rs.getString(8),rs.getTimestamp(6), rs.getInt("cnt") );
				list.add(data);
			}
			if(!list.isEmpty()) 
			return list;
		}catch(SQLException e) {
			System.out.println("board페이지 불러오기 실패");
			e.printStackTrace();
		}		
		return null;
	}
	...
	private void createTable() {
		...
		try {
			...
			System.out.println("board 테이블확인 실패");
			e.printStackTrace();
		}
		// 테이블 만들기
