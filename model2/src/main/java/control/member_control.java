...
public class member_control extends HttpServlet {
	...
	private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cmd.equals("signup")) { // 요청 파라미터가 signup일경우 회원가입페이지 요청한거
			...
			// 회원가입 페이지를 제공하기위해 request객체를 이용한것일뿐 반드시 이와같은 방법이 아니여도된다.
			// 사용자 요청에 의한 페이지 제공은 정적페이지(html,jsp등) 그대로 제공할수도 있고
 ...
 *     response.sendRedirect가 있는데 사용자의 브라우저 주소를 변경하는 방법이라서 다시 한번 재요청이 들어온다.
 *     
