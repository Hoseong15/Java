...
public class board_reply implements board_action {
	...
	public String action(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		reply b = new reply(bid, user.getNum(), comment, user.getName() );
		dao.insertreply(b);
		...
		// 메일보내기
		final String username="dlsghtjd3982@naver.com";
		final String password="5RZ7YQY84DGV";
		...
		try { // 메세지작성
			...
			msg.setRecipients(Message.RecipientType.TO, InternetAddress.parse(org.getAnswer_mail())); // 받는 이메일
			msg.setSubject(org.getTitle()+ "게시글에 답글 등록"); // 제목
			msg.setText(org.getTitle()+"의 글에 "+ user.getName() +"답글이 등록되었습니다."
			+"확인하시려면 아래 링크를 클릭해주세요.\n"+"http://localhost:8080/board_read.do?id="+bid, 
						"UTF-8"); // 내용
			Transport.send(msg);
