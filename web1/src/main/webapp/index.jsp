...
<%-- 스크립트 주석
 ...
 session - 클라이언트가 서버에 접속했을때 연결을 위해 정보를 저장하는 객체
 application - 서버의 정보를 가지고 있는 객체, 서버에 한개만 존재하는 객체다
 out - 출력 용 객체
 page - 특정 페이지의 서블릿객체, jsp페이지의 다른 내장객체들에 접근이 가능하다.
 pageContext - 현재 jsp페이지에 실행 가능한 정보를 저장하고있는 객체, 다음 내장객체 접근 가능
...
<%
	...
	} // 로그인 성공시 user변수는 값을 가지고, 로그인 하지 않은 상태에서는 user변수는 null을 가진다
	
	String part = null; // index.jsp에 part 파라미터 값이 들어오면 part파라미터 값을 가지게되고 part파라미터가 없다면
	// null값을 가진다. part 파라미터는 페이지 이동시에만 값을 가진다.
